...
export default function TrainerChangePassword({navigation, route}) {
  ...
  const processChangePassword = () => {
    ...
    Promise.resolve()
      ...
      .then(() => {
        var changeData = new FormData();
        changeData.append('changepassword', '1'),
          changeData.append('old_password', password),
          changeData.append('new_password', inputs.newPassword);
        changeData.append('user_id', id);
